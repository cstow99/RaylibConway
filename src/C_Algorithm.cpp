...
// Fixes the 'memset was not declared in this scope' seen in some compilers
#include <cstring>

C_Algorithm::C_Algorithm(Grid& grid, Vec2<int> size)
	: _grid(grid), _width(size.GetX()), _height(size.GetY()),
	  _output(new int[_width * _height]), _state(new int[_width * _height])
{
	memset(_output, 0, _width * _height * sizeof(int));
	memset(_state, 0, _width * _height * sizeof(int));
}

void C_Algorithm::Clear() {
	for (int i = 0; i < _width * _height; i++) {
		_state[i] = 0;
	}
}

void C_Algorithm::Generate(GenMode mode) {
	Clear();
